...
std::istream &CSFMDataPattern::read(std::istream &is) {
  is >> timestamp >> txyz[0] >> txyz[1] >> txyz[2] >> qxyzw[0] >> qxyzw[1] >>
      qxyzw[2] >> qxyzw[3];
  return is;
}
std::ostream &CSFMDataPattern::print(std::ostream &os) const {
  ...
  os << std::setprecision(12) << timestamp << delim << txyz[0] << delim
     << txyz[1] << delim << txyz[2] << delim << std::setprecision(6) << qxyzw[0]
     << delim << qxyzw[1] << delim << qxyzw[2] << delim << qxyzw[3] << endl;
  return os;
}

void loadCSFMOutput(string csfmFile,
                    vector<pair<double, Eigen::Vector3d> > &vTimeAndPose) {
  ...
  while (!stream.eof()) {
    ...
    line_str >> pat;
    Eigen::Vector3d txyz;
    txyz << pat.txyz[0], pat.txyz[1], pat.txyz[2];
...
void saveCSFMOutput(
    string csfmFile,
    const vector<pair<double, Eigen::Vector3d> > &vTimeAndPose) {
  ofstream stream(csfmFile);
  CSFMDataPattern pat;

  for (auto it = vTimeAndPose.begin(), ite = vTimeAndPose.end(); it != ite;
       ++it) {
...
std::istream &operator>>(std::istream &is,
  ...
  char delim;
  is >> rhs.GPS_TFlags >> delim >> rhs.GPS_Week >> delim >> rhs.GPS_TOW >>
      delim >> rhs.IMU_Timestamp >> delim >> rhs.IMU_Sync_Flags >> delim >>
      rhs.IMU_Sync_Seconds >> delim >> rhs.IMU_Sync_Nanoseconds >> delim >>
...
std::ostream &operator<<(std::ostream &os,
  ...
  os << rhs.GPS_TOW << delimiter << rhs.awxyz[0] << delimiter << rhs.awxyz[1]
     << delimiter << rhs.awxyz[2] << delimiter << rhs.awxyz[3] << delimiter
     << rhs.awxyz[4] << delimiter << rhs.awxyz[5];
  return os;
}
...
std::ostream &operator<<(std::ostream &os,
  ...
  const char delimiter = ' ';
  os << rhs.index << rhs.GPS_TOW << delimiter << rhs.awxyz[0] << delimiter
     << rhs.awxyz[1] << delimiter << rhs.awxyz[2] << delimiter << rhs.awxyz[3]
     << delimiter << rhs.awxyz[4] << delimiter << rhs.awxyz[5];
  return os;
}

std::istream &operator>>(std::istream &is,
                         IMUGrabber::IndexedPlainTextPattern &rhs) {
  is >> rhs.index >> rhs.GPS_TOW >> rhs.awxyz[0] >> rhs.awxyz[1] >>
      rhs.awxyz[2] >> rhs.awxyz[3] >> rhs.awxyz[4] >> rhs.awxyz[5];
  rhs.GPS_TOW *= 0.001;
...
std::ostream &operator<<(std::ostream &os,
  ...
  for (const std::pair<int, Eigen::Vector3d> &obs : rhs.measurements) {
    os << obs.first << delimiter << (obs.second)[0] << delimiter
       << (obs.second)[1] << delimiter << (obs.second)[2] << ' ';
  }
  return os;
}
...
std::ostream &operator<<(std::ostream &os,
  ...
  const char delimiter = ',';
  os << rhs.timeNanos << delimiter << rhs.gyro_accel[0] << delimiter
     << rhs.gyro_accel[1] << delimiter << rhs.gyro_accel[2] << delimiter
     << rhs.gyro_accel[3] << delimiter << rhs.gyro_accel[4] << delimiter
     << rhs.gyro_accel[5];
  return os;
}
...
std::istream &operator>>(std::istream &is, IMUGrabber::KalibrCsvPattern &rhs) {
  char delimiter;
  is >> rhs.timeNanos >> delimiter >> rhs.gyro_accel[0] >> delimiter >>
      rhs.gyro_accel[1] >> delimiter >> rhs.gyro_accel[2] >> delimiter >>
      rhs.gyro_accel[3] >> delimiter >> rhs.gyro_accel[4] >> delimiter >>
      rhs.gyro_accel[5];
  return is;
...
void IMUGrabber::print(const std::string message) {
  ...
  for (const Eigen::Matrix<double, 7, 1> &item : measurement) {
    std::cout << std::setprecision(12) << item[0] << " "
...
bool IMUGrabber::getObservation(double tk) {
  ...
  if (reinit_data) {  // grab IMU data until its timestamp is greater than
                      // t(startIndex)
    while (!stream.eof()) {
      getline(stream, tempStr);
      ...
      if (file_type == MicroStrainCSV) {
        ...
        if (pat.measurements.size() >= 2 &&
            pat.measurements[0].first ==
                SensorStreamCSVPattern::accelerometerIndex &&
      ...
      } else if (file_type == KalibrCsv) {
        IMUGrabber::KalibrCsvPattern pat;
        line_str >> pat;

        transMat[0] = nanoIntToSecDouble(pat.timeNanos);
        for (int j = 1; j < 4; ++j) transMat[j] = pat.gyro_accel[j + 2];
        for (int j = 4; j < 7; ++j) transMat[j] = pat.gyro_accel[j - 4];
      }

      ++lineNum;
      if (transMat[0] > tk) break;
    ...
    if (tk < transMat[0]) {
      tkm1 = tk;
      std::cerr
          << "Try to obtain inertial measurements until "
          << std::setprecision(12) << tk
          << " which is however before the latest retrieved entry of time "
          << transMat[0] << std::endl;
      std::cerr << "from the file stream. A possible cause is a gap in the "
                   "inertial data which can be filled by linear interpolation."
                << std::endl;
  ...
  if (transMat[0] > tk)
    ...
    is_measurement_good = false;  // this can heppen when the timestamp goes
                                  // beyond those in the imu file.
...
bool StatesGrabber::getObservation(double tk) {
  // Assume each line in States file: GPS TOW, i.e., t(k), position of sensor in
  // world frame,
  // quaternion wxyz from sensor to world frame, velocity of sensor in world
  ...
  if (tk - tkm1 > epsilonTime) {
    while (!stream.eof()) {
      ...
      getline(stream, tempStr);  // remove the remaining part, this works even
                                 // when it is empty
      tkm1 = precursor;
      if (std::fabs(precursor - tk) < epsilonTime) {
        ...
        std::cerr
            << "Warn: current time exceeds required time in state grabber "
            << std::endl;
        is_measurement_good = false;
        break;
      }
    ...
    std::cerr << "Warn: current time well above required time in state grabber "
              << std::endl;
    is_measurement_good = false;
  }
...
bool StatesGrabber::getNextObservation() {
  // Assume each line in States file: GPS TOW, i.e., t(k), position of sensor in
  // world frame,
  // quaternion xyzw from sensor to world frame, velocity of sensor in world
  // frame, accelerometer biases, and gyro biases
  double precursor = 0;  // used to read in something tentatively
  string tempStr;
  ...
  if (!stream.eof()) {
    ...
    measurement[0] = precursor;
    if (delimiter == ' ') {
      for (size_t j = 1; j < measurement.size(); ++j) stream >> measurement[j];
    } else {
      char trashbin = '0';
      for (size_t j = 1; j < measurement.size(); ++j)
        stream >> trashbin >> measurement[j];
    ...
    if (precursor > tkm1) {
      ...
      std::cerr << "Warn: current entry time is less than previous one in "
                   "state grabber "
                << std::endl;
      is_measurement_good = false;
    }
...
void loadKITTIPoses(
    string file,
    vector<pair<double, Eigen::Matrix<double, 3, 4> > > &vTimeAndPose) {
  ...
  while (sg.getNextObservation()) {
    ...
    currentPose << sg.measurement[0], sg.measurement[1], sg.measurement[2],
        sg.measurement[3], sg.measurement[4], sg.measurement[5],
        sg.measurement[6], sg.measurement[7], sg.measurement[8],
        sg.measurement[9], sg.measurement[10], sg.measurement[11];

    std::pair<double, Eigen::Matrix<double, 3, 4> > meas =
        make_pair(counter * 0.1f, currentPose);
    vTimeAndPose.push_back(meas);
    ++counter;
  }
...
void computeKITTIRf2s() {
  ...
  string file =
      "/media/jhuai/Seagate/jhuai/kitti/data_odometry_poses/dataset/poses/"
      "00.txt";
  ...
  for (size_t i = 1; i < vTimeAndPose.size(); ++i) {
    Eigen::Matrix3d Rf2s =
        vTimeAndPose[i].second.block<3, 3>(0, 0).transpose() *
        vTimeAndPose[i - 1].second.block<3, 3>(0, 0);
