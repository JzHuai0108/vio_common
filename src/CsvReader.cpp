...
Eigen::VectorXd CsvReader::OkvisOutputPattern::toEigenVector() {
  ...
  for (int j = 3; j < 6; ++j) transMat[j] = p_WS_[j - 3];
  for (int j = 6; j < 10; ++j) transMat[j] = q_WS_[j - 6];
  for (int j = 10; j < 19; ++j) transMat[j] = sb_WS_[j - 10];
  return transMat;
}
double CsvReader::OkvisOutputPattern::getTime() { return nsec_ * 1e-9 + sec_; }
...
CsvReader::CsvReader(const std::string file, const OUTPUT_FILE_TYPE fileType)
    : DataGrabber(file) {
  std::string tempStr;
...
CsvReader::OkvisOutputPattern::OkvisOutputPattern()
    : pq_SC_{0, 0, 0, 0, 0, 0, 0} {}

std::ostream& CsvReader::OkvisOutputPattern::print(std::ostream& os) const {
  char delim = ',';

  os << sec_ << "." << std::setw(9) << std::setfill('0') << nsec_ << delim
     << frameIdInSource << delim << p_WS_[0] << delim << p_WS_[1] << delim
     << p_WS_[2] << delim << q_WS_[0] << delim << q_WS_[1] << delim << q_WS_[2]
     << delim << q_WS_[3] << delim << sb_WS_[0] << delim << sb_WS_[1] << delim
     << sb_WS_[2] << delim << sb_WS_[3] << delim << sb_WS_[4] << delim
     << sb_WS_[5] << delim << sb_WS_[6] << delim << sb_WS_[7] << delim
     << sb_WS_[8] << delim << pq_SC_[0] << delim << pq_SC_[1] << delim
     << pq_SC_[2] << delim << pq_SC_[3] << delim << pq_SC_[4] << delim
     << pq_SC_[5] << delim << pq_SC_[6];
...
std::istream& CsvReader::OkvisOutputPattern::read(std::istream& is) {
  ...
  if (is >> delim) {
    is >> pq_SC_[0] >> delim >> pq_SC_[1] >> delim >> pq_SC_[2] >> delim >>
        pq_SC_[3] >> delim >> pq_SC_[4] >> delim >> pq_SC_[5] >> delim >>
        pq_SC_[6];
  }
  return is;
}
...
void loadCsvData(
    string csvFile,
    vector<Eigen::Matrix<double, 19, 1>,
  ...
  CsvReader cg(csvFile, OKVIS_OUTPUT_FILE);
...
bool ViclamOutputReader::getNextObservation() {
  ...
  if (!stream.eof()) {
    ...
    transMat[2] = 0;
    for (int j = 3; j < 6; ++j) transMat[j] = pat_->p_WS_[j - 3];
    for (int j = 6; j < 10; ++j) transMat[j] = pat_->q_WS_[j - 6];
    for (int j = 10; j < 19; ++j) transMat[j] = pat_->sb_WS_[j - 10];
...
ViclamOutputReader::ViclamOutputReader(const std::string file)
    : DataGrabber(file) {
  transMat[0] = -1;
  measurement[0] = -1;
  //    std::string tempStr;
...
std::ostream& ViclamOutputReader::ViclamOutputPattern::print(
...
void loadViclamData(
    ...
    double startTime, double finishTime) {
  if (finishTime <= 0) finishTime = 1E20;
  ViclamOutputReader vg(viclamFile);
