...
GPSGrabber::GPSGrabber(const std::string file, double interval,
                       GPSSolutionType patType)
    : DataGrabber(file), interval_(interval) {
  transMat[0] = -1;
  measurement[0] = -1;
  std::string tempStr;
  getline(stream, tempStr);  // remove header part
  // N.B. we also delete the first line of observation
  while (tempStr.find('%') != std::string::npos) getline(stream, tempStr);
...
bool GPSGrabber::getObservation(double tk) {
  // Assume each line in gps_file: GPSWeek, TOW, ecef X,Y,Z, Q and no of satels,
  // and std ecef X,Y,Z
  int lineNum = 0;  // how many lines have been read in
  string tempStr;

  if (reinit_data) {  // grab data until its timestamp is greater than
                      // t(startIndex)
    while (!stream.eof()) {
      getline(stream, tempStr);
    ...
    if (abs(transMat[0] - tk) < interval_) {
      ...
      is_measurement_good = false;
      reinit_data = true;
      assert(false);  // this should never happen with continuous sensor
                      // readings
    }
  }
  tkm1 = tk;
...
std::ostream& GPSGrabber::RtklibPosPattern::print(std::ostream& os) const {
  ...
  os << GPS_Week << delim << GPS_TOW << delim << xyz_ecef[0] << delim
     << xyz_ecef[1] << delim << xyz_ecef[2] << delim << Q << delim << ns
     << delim << sdxyz[0] << delim << sdxyz[1] << delim << sdxyz[2] << delim
     << sdxy_yz_zx[0] << delim << sdxy_yz_zx[1] << delim << sdxy_yz_zx[2]
     << delim << age << delim << ratio;
  return os;
}
std::istream& GPSGrabber::RtklibPosPattern::read(std::istream& is) {
  is >> GPS_Week >> GPS_TOW >> xyz_ecef[0] >> xyz_ecef[1] >> xyz_ecef[2] >> Q >>
      ns >> sdxyz[0] >> sdxyz[1] >> sdxyz[2] >> sdxy_yz_zx[0] >>
      sdxy_yz_zx[1] >> sdxy_yz_zx[2] >> age >> ratio;
  return is;
}

std::ostream& GPSGrabber::RtklibPosPatternUTCECEF::print(
    std::ostream& os) const {
  char delim = ' ';
  os << ymd[0] << '/' << ymd[1] << '/' << ymd[2] << ' ' << hour << ':' << minute
     << ':' << second << delim << xyz_ecef[0] << delim << xyz_ecef[1] << delim
     << xyz_ecef[2] << delim << Q << delim << ns << delim << sdxyz[0] << delim
     << sdxyz[1] << delim << sdxyz[2] << delim << sdxy_yz_zx[0] << delim
     << sdxy_yz_zx[1] << delim << sdxy_yz_zx[2] << delim << age << delim
     << ratio;
  return os;
}

std::ostream& GPSGrabber::RtklibPosPatternTOWNED::print(
    std::ostream& os) const {
  char delim = ' ';
  os << GPS_Week << delim << GPS_TOW << delim << llh_geo[0] << delim
     << llh_geo[1] << delim << llh_geo[2] << delim << Q << delim << ns << delim
     << sdneu[0] << delim << sdneu[1] << delim << sdneu[2] << delim
     << sdne_eu_un[0] << delim << sdne_eu_un[1] << delim << sdne_eu_un[2]
     << delim << age << delim << ratio;
  return os;
}
...
std::istream& GPSGrabber::RtklibPosPatternTOWNED::read(std::istream& is) {
  is >> GPS_Week >> GPS_TOW >> llh_geo[0] >> llh_geo[1] >> llh_geo[2] >> Q >>
      ns >> sdneu[0] >> sdneu[1] >> sdneu[2] >> sdne_eu_un[0] >>
      sdne_eu_un[1] >> sdne_eu_un[2] >> age >> ratio;
  // convert NED to ECEF
  gpstk::Position d;
  ...
  xyz_ecef[1] = d.Y();
  xyz_ecef[2] = d.Z();
  Eigen::Matrix3d Ce2n = vio::llh2dcm(Eigen::Vector3d(
      llh_geo[0] * (M_PI / 180), llh_geo[1] * (M_PI / 180), llh_geo[2]));
  Eigen::Matrix3d Rneu;
  Rneu << copysign(sdneu[0] * sdneu[0], sdneu[0]),
      copysign(sdne_eu_un[0] * sdne_eu_un[0], sdne_eu_un[0]),
      copysign(sdne_eu_un[2] * sdne_eu_un[2], sdne_eu_un[2]),
      copysign(sdne_eu_un[0] * sdne_eu_un[0], sdne_eu_un[0]),
      copysign(sdneu[1] * sdneu[1], sdneu[1]),
...
std::istream& GPSGrabber::RtklibPosPatternUTCECEF::read(std::istream& is) {
  ...
  is >> ymd[0] >> delim >> ymd[1] >> delim >> ymd[2] >> hour >> delim >>
      minute >> delim >> second >> xyz_ecef[0] >> xyz_ecef[1] >> xyz_ecef[2] >>
      Q >> ns >> sdxyz[0] >> sdxyz[1] >> sdxyz[2] >> sdxy_yz_zx[0] >>
      sdxy_yz_zx[1] >> sdxy_yz_zx[2] >> age >> ratio;
  // convert of time
  gpstk::CivilTime civtime(ymd[0], ymd[1], ymd[2], hour, minute, second,
...
void testReadTOWECEF() {
  string tempStr =
      "1752 414790.200   40.003227423  -83.042986648   212.6010   2   7   "
      "0.0086   0.0090   0.0241  -0.0068   0.0101  -0.0108   0.20    2.6";
  string target =
      "1752 414790.200 592619.2852 -4856617.0869 4078396.7487 2 7 0.0085 "
      "0.0163 0.0200 0.0051 -0.0167 -0.0082 0.20 2.6";
  stringstream line_str(tempStr);
  ...
  cout << "Actual result " << std::endl
       << std::setprecision(10) << *pat << endl;
}
...
int gpstk_example1() {
  ...
  std::cout << "Expected gps week and second of week: 1870 315518.6"
            << std::endl;
  cout << "Calculted gpg week and second of week " << gpstime2.week << " "
       << setprecision(10) << gpstime2.sow << endl;
  ...
  cout << "Calculated civil time " << setprecision(6) << civtime3
       << " and civil time second " << civtime3.second << endl;
  return 0;
}
#else
std::istream& GPSGrabber::RtklibPosPatternTOWNED::read(std::istream& is) {
...
int gpstk_example1() { return 0; }
#endif

void loadGPSData(
    string gps_file,
    vector<Eigen::Matrix<double, 7, 1>,
           Eigen::aligned_allocator<Eigen::Matrix<double, 7, 1> > >& gpsdata,
  ...
  while (gg.getNextObservation()) {
    ...
    if (gps_obs[0] < startGPSTime || gps_obs[0] > finishGPSTime) continue;
