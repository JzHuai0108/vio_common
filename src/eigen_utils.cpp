...
void testReparameterize() {
  ...
  for (size_t jack = 0; jack < sizeof(distances) / sizeof(distances[0]);
    ...
    std::cout << "analytic jacobian - numerical jacobian " << std::endl
              << (jacobian - jacobian3) << std::endl;
    std::cout << "abrhoi and j" << abrhoi.transpose() << " "
              << abrhoj.transpose() << std::endl;
  }
  // infinity
  Eigen::Matrix3d Ri = Eigen::Matrix3d::Identity();
  ...
  std::cout << "analytic jacobian - numerical jacobian " << std::endl
            << (jacobian - jacobian3) << std::endl;
  std::cout << "abrhoi and j" << abrhoi.transpose() << " " << abrhoj.transpose()
            << std::endl;
