...
def main():
    ...
    if DEBUG:
        debug_dir = ioUtilities.setup_debug_dir(args.debug_dir)
        # Write raw frames.
...
def collect_data(args):
    ...
    with its.device.ItsSession() as cam:
        ...
        shutter_skews = {
            c['metadata']['android.sensor.rollingShutterSkew'] * NSEC_TO_MSEC
            for c in raw_caps
        }
...
def calculate_shutter_skew(frame, led_time, frame_num=None, debug_dir=None):
    ...
    if debug_dir is not None:
        cv2.imwrite('%s/contour/%03d.png' % (debug_dir, frame_num),
                    contour_img)
        cv2.imwrite('%s/mono/%03d.png' % (debug_dir, frame_num), mono_img)

    largest_cluster, cluster_percentage = find_largest_cluster(
        contours, scratch_img)
...
def find_contours(img):
    ...
    red_img = img[:, :, 2]
    _, thresh = cv2.threshold(red_img, 0, 255,
...
def proximity_clusters(contours):
    ...
    for i, curr in enumerate(circles):
        close_circles = [
            j for j, p in enumerate(circles)
            if i != j and curr.distance_to(p) < CLUSTER_DISTANCE
        ]
        if close_circles:
