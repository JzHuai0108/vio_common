 ...
 public:
  ...
  class OkvisOutputPattern : public LinePattern {
   ...
   public:
    ...
    toEigenVector();  // sec, nsec, frameid, pxyz, qxyzw, speed and bg ba
    virtual double getTime();

   public:
    // each line of the csv file contains timestamp(unit nsec) frameIdInSource
    // p_WS_W_x p_WS_W_y p_WS_W_z
  ...
  CsvReader(const std::string file,
            const OUTPUT_FILE_TYPE fileType = OKVIS_OUTPUT_FILE);
  virtual ~CsvReader();

  bool getNextObservation();
  Eigen::VectorXd
      transMat;  // temporary sensor reading which also holds data at t(p(k)-1);
  Eigen::VectorXd measurement;  // the current measurement
  LinePattern *pat_;
};

// use CsvReader to load data from the okvis output csv file in which each line
// contains frameIdInSource
void loadCsvData(
    std::string csvFile,
...
class ViclamOutputReader : vio::DataGrabber {
 public:
  ...
  class ViclamOutputPattern {
   ...
   public:
    ...
    friend std::ostream &operator<<(std::ostream &os,
                                    const ViclamOutputPattern &rhs) {
      return rhs.print(os);
    }
    friend std::istream &operator>>(std::istream &is,
                                    ViclamOutputPattern &rhs) {
      return rhs.read(is);
    }
    virtual ~ViclamOutputPattern() {}

   public:
    // each line of the viclam output file contains timestamp(unit nsec)
    // p_WS_W_x p_WS_W_y p_WS_W_z
    // q_WS_x q_WS_y q_WS_z q_WS_w v_WS_W_x v_WS_W_y v_WS_W_z b_g_x b_g_y b_g_z
    // b_a_x b_a_y b_a_z
  ...
  ViclamOutputReader(const std::string file);
  virtual ~ViclamOutputReader();

  bool getNextObservation();
  Eigen::Matrix<double, 19, 1>
      transMat;  // temporary sensor reading which hold the last line of data
  Eigen::Matrix<double, 19, 1>
      measurement;  // sec, nsec, placeholder, pxyz, qxyzw, speed and bg ba
  ViclamOutputPattern *pat_;
};
// load all data of a viclam output file
void loadViclamData(
    std::string viclamFile,
    std::vector<Eigen::Matrix<double, 19, 1>,
                Eigen::aligned_allocator<Eigen::Matrix<double, 19, 1> > >
        &viclamData,
    double startTime = 0, double finishTime = 0);
