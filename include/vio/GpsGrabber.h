 ...
 public:
  ...
  class RtklibPosPattern {
   ...
   public:
    ...
    int ns;
    double sdxyz[3];
    double sdxy_yz_zx[3];  // square root of the absolute values of xy, yz, zx
  ...
  // TODO: besides removing the header part, the first line of GPS observation
  // is also deleted in the constructor
  GPSGrabber(const std::string file, double interval = 1e-5,
             GPSSolutionType patType = GPSSolutionType::TOW_ECEF);
  virtual ~GPSGrabber();
  ...
  Eigen::Matrix<double, 7, 1> measurement;  // GPS TOW, XYZ ECEF, Q, number of
                                            // satellites, std XYZ in ECEF
  RtklibPosPattern* pat_;
  const double interval_;  // maximum allowed difference between retrieved and
                           // expected timestamp
};
void loadGPSData(
    std::string gps_file,
