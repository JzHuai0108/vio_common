/*
 * file: sort.h
 * https://www.alecjacobson.com/weblog/?p=1527
 */
...
template< class T >
void reorderMatrices(
  std::vector<T, Eigen::aligned_allocator<T>> & unordered,
  std::vector<size_t> const & index_map,
  std::vector<T, Eigen::aligned_allocator<T>> & ordered);
...
void reorderMatrices(
  ...
  std::vector<size_t> const & index_map,
    ...
    // copy for the reorder according to index_map, because unsorted may also be
    // sorted
    std::vector<T, Eigen::aligned_allocator<T>> copy = unordered;
    ordered.resize(index_map.size());
    for(size_t i = 0; i<index_map.size();i++)
    {
      ordered[i] = copy[index_map[i]];
