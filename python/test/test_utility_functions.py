...
def test_decide_time_index_and_unit():
    ...
    lines = ["1021383336666000, 3", "1021383336766000, 4"]
    index, unit, r_index = utility_functions.decide_time_index_and_unit(
        lines, ',')
    assert index == 0 and unit == 'ns' and r_index == 2

    lines = ["34293842, 12389.0121930", "34293856, 12389.0151930"]
    index, unit, r_index = utility_functions.decide_time_index_and_unit(
        lines, ',')
    assert index == 1 and unit == 's' and r_index == 2

    lines = ["1021383.336666000, 332189021", "1021383.346666000, 332189031"]
    index, unit, r_index = utility_functions.decide_time_index_and_unit(
        lines, ',')
    assert index == 0 and unit == 's' and r_index == 2

    lines = ["123890.121930, 123.23910", "123890.131930, 233.23910"]
    index, unit, r_index = utility_functions.decide_time_index_and_unit(
        lines, ',')
    assert index == 0 and unit == 's' and r_index == 1

    lines = [
        "1021383336666000, 2139342.0342", "1021383336766000, 2139542.0332"
    ]
    index, unit, r_index = utility_functions.decide_time_index_and_unit(
        lines, ',')
...
def test_interpolate_imu_data():
    time_gyro_data = np.array([[0.000333, -0.034319, 0.036195, -0.054551],
    ...
    time_acc_data = np.array([[0.007932, -0.728685, 8.044571, 6.028114],
