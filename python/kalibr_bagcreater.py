...
# The video time file has the timestamp for every video frame according to the IMU clock.
...
def parse_args():
    ...
    if len(sys.argv) < 2:
        ...
        msg += 'Example usage 1.1: {} --folder tango/android/export/ --imu gyro_accel.csv ' \
               '--output_bag awsome.bag\n'.format(sys.argv[0])

        msg += "dataset or export has at least one subfolder cam0 which contains nanosecond named images"

        msg += 'Example usage 2: {} --video marslogger/ios/dataset/IMG_2805.MOV ' \
               '--imu marslogger/ios/dataset/gyro_accel.csv --video_time_file ' \
               'marslogger/ios/dataset/movie_metadata.csv ' \
               '--output_bag marslogger/ios/dataset/IMG_2805.bag\n'.format(sys.argv[0])

        msg += 'Example usage 3: {} --video marslogger/android/dataset/IMG_2805.MOV ' \
               '--imu marslogger/android/dataset/gyro_accel.csv --video_time_file ' \
               'marslogger/android/dataset/frame_timestamps.txt ' \
               '--output_bag marslogger/android/dataset/IMG_2805.bag\n '.format(sys.argv[0])

        msg += 'Example usage 4: {} --video advio-01/iphone/frames.MOV ' \
               '--imu advio-01/iphone/gyro.csv advio-01/iphone/accelerometer.csv' \
               ' --video_time_file advio-01/iphone/frames.csv  ' \
               '--output_bag advio-01/iphone/iphone.bag\n '.format(sys.argv[0])
...
def get_image_files_from_dir(input_dir):
    ...
    if os.path.exists(input_dir):
        for path, names, files in os.walk(input_dir):
            for f in files:
...
def create_imu_message_time_string(timestamp_str, omega, alpha):
    ...
    return create_imu_message(timestamp, omega, alpha)
...
def play_video(video_filename):
    ...
    while cap.isOpened():
        ...
        if video_frame_id != current_id:
            ...
            print("Likely reached end of video file. Note finish_id {}".format(finish_id))
            break        
           
        # time_frame= cap.get(cv2.CAP_PROP_POS_MSEC)/1000.0
        # print('currentFrameId {} and timestamp in video {:.9f}'.format(current_id, time_frame))
        ret, frame = cap.read()
        if frame is None:
        ...
        if current_id > finish_id:
            print('Exceeding finish_id %d' % finish_id + ', break the video stream')
            break            
...
def write_video_to_rosbag(bag, video_filename, video_from_to,
    ...
    literalvideofromto.append(max(float(start_id)/rate - 1.0, 0.0) + video_time_offset)
    literalvideofromto.append(float(finish_id)/rate + 1.0 + video_time_offset)
...
def main():
    ...
    if parsed.video is not None:
        ...
        videotimerange = write_video_to_rosbag(bag, parsed.video, parsed.video_from_to,
                                               video_time_offset,
            ...
            with open(imufile, 'rb') as csvfile:
                ...
                for row in reader:
                    if utility_functions.is_header_line(row):
                        continue
                    imumsg, timestamp = create_imu_message_time_string(row[0], row[1:4], row[4:7])
