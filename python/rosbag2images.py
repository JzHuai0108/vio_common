...
"""
adapted from https://raw.githubusercontent.com/mlaiacker/rosbag2video/master/rosbag2video.py
rosbag2video.py
...
class RosVideoWriter():
    ...
    def write_image(self, msg, t):
        if not self.img_out_dir:
            self.img_out_dir = self.opt_out_file.split('.')[0]
            if not os.path.exists(self.img_out_dir):
                os.mkdir(self.img_out_dir)

        imagename = os.path.join(self.img_out_dir, '{}.png'.format(t))
        cv2.imwrite(imagename, msg)
