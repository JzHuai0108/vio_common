...
def parseArgs():
    ...
    parser = argparse.ArgumentParser(
        description=
        'Convert the poses in a csv file of TUM_RGBD or KALIBR format to ROS'
        ' path messages in a rosbag.')
    ...
    parser.add_argument('--outfile',
                        default="infile.bag",
                        help='ROS bag file (default: %(default)s)')

    parser.add_argument(
        '--plus_time_file',
        default="",
        help='The file of the time offset in secs added to the timestamps in'
        ' the csv file. If not provided, the time offset is set to zero.'
        ' (default: %(default)s)')
    ...
    parser.add_argument(
        ...
        default="vins",
        help=
        'The signiture of the source frame of the poses (default: %(default)s)'
    )
...
def bag_tf_poses(timed_pos_quat,
    ...
    for index, entry in enumerate(timed_pos_quat):
        ...
        # see https://wiki.ros.org/rviz/DisplayTypes/Marker#Triangle_List_.28TRIANGLE_LIST.3D11.29_.5B1.1.2B-.5D for drawing complex markers  # pylint:disable=line-too-long
